...
{
    public record PushConstants(int offset, int size) {}
    public record VertexShaderStage(String filename, boolean compile, InputData[] inputDataList, Attribute[] attributes, @Nullable PushConstants pushConstants)
    {
        public record InputData(int size, boolean per_vertex) {}
        public record Attribute(int buffer_index, int format, int offset) {}
    }
    public record ShaderStage(String filename, boolean compile, @Nullable PushConstants pushConstants) {}
    public record Description(VertexShaderStage vertex_shader, ShaderStage fragment_shader, @Nullable ShaderStage tessellation_control_shader, @Nullable ShaderStage tessellation_evaluation_shader, @Nullable ShaderStage geometry_shader,
                              Either<VkViewport.Buffer, Integer> viewports, Either<VkRect2D.Buffer, Integer> scissors, int num_samples) {}
    ...
    public GraphicsPipeline(VkDevice device, long renderPass, Description description) throws VulkanException
    {
        ...
        try (FrameAllocator allocator = FrameAllocator.takeAndPush())
        {
            ...
            VkPipelineShaderStageCreateInfo.Buffer pipelineShaderStageCreateInfos;
            VkPushConstantRange.Buffer pushConstantRanges;
            {
                ByteBuffer entryPoint = allocator.UTF8("main");
                int i = 2;
                if (description.tessellation_control_shader != null) i++;
                if (description.tessellation_evaluation_shader != null) i++;
                if (description.geometry_shader != null) i++;
                pipelineShaderStageCreateInfos = VkPipelineShaderStageCreateInfo.calloc(i, allocator)
                        .apply(0, ci -> ci
                                .sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO)
                                .stage(VK_SHADER_STAGE_VERTEX_BIT)
                                .module(this.createShaderModule(description.vertex_shader.filename, entryPoint, shaderc_vertex_shader, description.vertex_shader.compile))
                                .pName(entryPoint))
                        .apply(1, ci -> ci
                                .sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO)
                                .stage(VK_SHADER_STAGE_FRAGMENT_BIT)
                                .module(this.createShaderModule(description.fragment_shader.filename, entryPoint, shaderc_fragment_shader, description.fragment_shader.compile))
                                .pName(entryPoint));
                int j = 2;
                if (description.tessellation_control_shader != null)
                {
                    pipelineShaderStageCreateInfos.apply(j++, ci -> ci
                            .sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO)
                            .stage(VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT)
                            .module(this.createShaderModule(description.tessellation_control_shader.filename, entryPoint, shaderc_tess_control_shader, description.tessellation_control_shader.compile))
                            .pName(entryPoint));
                }
                if (description.tessellation_evaluation_shader != null)
                {
                    pipelineShaderStageCreateInfos.apply(j++, ci -> ci
                            .sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO)
                            .stage(VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT)
                            .module(this.createShaderModule(description.tessellation_evaluation_shader.filename, entryPoint, shaderc_tess_evaluation_shader, description.tessellation_evaluation_shader.compile))
                            .pName(entryPoint));
                }
                if (description.geometry_shader != null)
                {
                    pipelineShaderStageCreateInfos.apply(j++, ci -> ci
                            .sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO)
                            .stage(VK_SHADER_STAGE_GEOMETRY_BIT)
                            .module(this.createShaderModule(description.geometry_shader.filename, entryPoint, shaderc_geometry_shader, description.geometry_shader.compile))
                            .pName(entryPoint));
                }

                if (description.vertex_shader.pushConstants == null) i--;
                if (description.fragment_shader.pushConstants == null) i--;
                pushConstantRanges = VkPushConstantRange.calloc(i, allocator);
                j = 0;
                if (description.vertex_shader.pushConstants != null)
                {
                    pushConstantRanges.apply(j++, range -> range
                            .offset(description.vertex_shader.pushConstants.offset)
                            .size(description.vertex_shader.pushConstants.size)
                            .stageFlags(VK_SHADER_STAGE_VERTEX_BIT));
                }
                if (description.fragment_shader.pushConstants != null)
                {
                    pushConstantRanges.apply(j++, range -> range
                            .offset(description.fragment_shader.pushConstants.offset)
                            .size(description.fragment_shader.pushConstants.size)
                            .stageFlags(VK_SHADER_STAGE_FRAGMENT_BIT));
                }
                if (description.tessellation_control_shader != null && description.tessellation_control_shader.pushConstants != null)
                {
                    pushConstantRanges.apply(j++, range -> range
                            .offset(description.tessellation_control_shader.pushConstants.offset)
                            .size(description.tessellation_control_shader.pushConstants.size)
                            .stageFlags(VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT));
                }
                if (description.tessellation_evaluation_shader != null && description.tessellation_evaluation_shader.pushConstants != null)
                {
                    pushConstantRanges.apply(j++, range -> range
                            .offset(description.tessellation_evaluation_shader.pushConstants.offset)
                            .size(description.tessellation_evaluation_shader.pushConstants.size)
                            .stageFlags(VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT));
                }
                if (description.geometry_shader != null && description.geometry_shader.pushConstants != null)
                {
                    pushConstantRanges.apply(j++, range -> range
                            .offset(description.geometry_shader.pushConstants.offset)
                            .size(description.geometry_shader.pushConstants.size)
            ...
            pipelineShaderStageCreateInfos.forEach(ci -> vkDestroyShaderModule(this.device, ci.module(), null));
        }
    }
