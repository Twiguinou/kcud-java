...
{
    private final Map<String, String[]> m_registeredArgs;

    public ProgramArguments(final String[] args)
    {
        this.m_registeredArgs = new HashMap<>();
        for (String arg : args)
        {
            ...
            if (!this.m_registeredArgs.containsKey(splitted[0]))
            {
                this.m_registeredArgs.put(splitted[0], splitted[1].split(","));
            }
        }
    }
    ...
    public int[] getArgValues(final String arg_s, int[] dest)
    {
        final String[] args = this.m_registeredArgs.get(arg_s);
        if (args != null)
        {
            for (int i = Math.min(args.length, dest.length) - 1; i >= 0; i--)
            {
                dest[i] = Integer.parseInt(args[i]);
            }
        }
        return dest;
    }
